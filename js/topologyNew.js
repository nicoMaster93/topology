...
var topology = {
            ...
            if(this.endtopology[this.domlastActiveTp]){
                ...
                setTimeout(() => {
                    ...
                    if(prefix==='pri_'){
                        ...
                        $(`#summary_tp_${this.domlastActiveTp}`).removeClass('active');
                        $(`#backBtnTp_${this.domlastActiveTp}`).click()
                    }
        ...
        if(typeof data.children != 'undefined' && data.children.length > 0){
            if(prefixLayout == 'deta_' && (typeof dataBase.viewDetail != 'undefined' && dataBase.viewDetail == true)){
                ...
                var node = `<li  data-node="${encodeData}" id="parent_li_${idContainerNodes}" class="li_tp_node">
                                <div data-idnode="${data.id}" ${statusNde} data-tpid="${this.domlastActiveTp}" data-prefix="${prefixLayout}" data-parentnodetp="#li_${dataParentNode.replace('#','')}" onmousedown="topology.iniPress()" onmouseup="topology.actionsByNode(this,'${prefixLayout}',event)" id="li_${idContainerNodes}" data-lvl="${data.levelTp}" class="tp_node"><span ${imgCssNde} >${labelNode}</span></div>
            ...
            if(prefixLayout == 'deta_' && (typeof dataBase.viewDetail != 'undefined' && dataBase.viewDetail == true)){
                ...
                var node = `<li data-node="${encodeData}" id="parent_li_${idContainerNodes}" class="li_tp_node">
                                <div ${statusNde} data-idnode="${data.id}" data-tpid="${this.domlastActiveTp}" data-prefix="${prefixLayout}" data-parentnodetp="#li_${dataParentNode.replace('#','')}" onmousedown="topology.iniPress()" onmouseup="topology.actionsByNode(this,'${prefixLayout}',event)" id="li_${idContainerNodes}" data-lvl="${data.levelTp}" class="tp_node not-child"><span ${imgCssNde} >${labelNode}</span></div> 
                            </li>`;  
            }
        }
    ...
    /* v1_SBL */
    getLocationNode(id,type,prefix,is_sibling){
        let obj  = this.elementID[this.domlastActiveTp];
        ...
        posicion.left = ( (posicion.left - ((positionContainer.left ) + 1.5)) + widthNode ) - this.paddingLeft ;
        // posicion.left = ( (posicion.left - ((positionContainer.left ) + 1.5)) + widthNode );
        posicion.top = (posicion.top - (positionContainer.top + 2));

        // if(!obj.showDetail){ /* code */ }
        if(prefix == 'pri_'){
            posicion.x = (posicion.left + .7);
            if(type == 'origen'){
                posicion.y = posicion.top + (heightNode + 1.5); /* + 17;*/
            }else if(type == 'destino'){
                posicion.y = (posicion.top + 2) - (heightNode / 2); /* - 15;*/
            }
            /* [ v1_SBL, v1_PRNS ]  */
            if(is_sibling && is_sibling !== 'MULTI_PARENTS'){
                posicion.y = (posicion.top - 2) + (heightNode / 2);
            }else if(is_sibling === 'MULTI_PARENTS'){
                // posicion.y = (posicion.top - 2) + (heightNode / 2);
                if(type == 'origen'){
                    posicion.x = posicion.left;
            ...
            if(is_sibling){
                /* Code siblings */
                posicion.y = (posicion.top + (heightNode / 2));
                console.log('Code siblings', id, widthNode, heightNode,)
            }
        }
        return posicion;
    },
    ...
    createLinesPP(idNodo,prefix,goNode=false,colorLine=false){
        ...
        var infoNode = decode($(idNodo).parents('li').data('node'));
        ...
        /* v1_SBL *//* Valido si el nodo tiene relacion con hermandos */
        if(infoNode.sibling !== undefined && $(`[data-idnode="${infoNode.sibling}"]`).length > 0 ){
            
            /* Filtro para que me retorne el id segun el contenedor */
            // this.log_error($(`[data-idnode="${infoNode.sibling}"]`))
            var idDestinoDom = $(`[data-idnode="${infoNode.sibling}"]`).filter( function(index){
                if ($(this).attr('id').indexOf(prefix) != -1) 
                    return $(this).attr('id');
            });
            var idDestino = `#${ $(idDestinoDom[0]).prop("id") }`;
            var id_line =  `#li_line_svg_${infoNode.sibling}-${infoNode.id}_${prefix}`;
            var id_line2 =  `#li_line_svg_${infoNode.id}-${infoNode.sibling}_${prefix}`;
            /* Valido que la linea no este de ningun lado de los hermanos */
            
            if($(id_line).length > 0 || $(id_line2).length > 0){
                this.log_error(['ya existe conexion', id_line, id_line2])
            }else{
                this.newLineSvg(id_line,idNodo,idDestino,id_svg,container,containerRef,prefix,colorLine,true);
            }

            // this.log_error([infoNode, $(`[data-idnode="${infoNode.sibling}"]`)])
            // this.log_error([
            //     'drawLine pintar linea entre hermanos', 
            //     idNodo, 
            //     $(idNodo).parents('li').data(), 
            //     decode($(idNodo).parents('li').data('node')), 
            //     $(idNodo).data() 
            // ]);
            // if (is_sibling)  debugger;
            
            

            /* this.centerNodeSection(container,$('.tp_node')[0]); */
        }
        this.log_error(infoNode)
        /* v1_PRNS *//* Valido si el nodo tiene papas en otro lado */
        if(infoNode.parents !== undefined && infoNode.parents.length > 0){
            
            /* Filtro para que me retorne el id segun el contenedor */
            // debugger
            for (let index = 0; index < infoNode.parents.length; index++) {
                const element = infoNode.parents[index];
                const id_line = `li_line_svg_child_${infoNode.id + `-` + element }_${prefix}`;
                const idDestinoDom = $(`[data-idnode="${element}"]`).filter( function(index){
                    if ($(this).attr('id').indexOf(prefix) != -1) 
                        return $(this).attr('id');
                });

                var idDestino = `#${ $(idDestinoDom[0]).prop("id") }`;
                this.newLineSvg(id_line,idDestino,idNodo,id_svg,container,containerRef,prefix,colorLine,'MULTI_PARENTS');
            }/* Fin for */
    ...
    /* v1_SBL */
    newLineSvg(id_line,idOrigine,idDestino,id_svg,container,containerRef,prefix,colorLine=false,is_sibling=false){
        ...
        var locationOrigine = this.getLocationNode(idOrigine,'origen',prefix,is_sibling);
        var locationDestino = this.getLocationNode(idDestino,'destino',prefix,is_sibling);
    ...
    iniPress(){
        this.elementID[this.domlastActiveTp].pressIni = new Date();
        console.log(this.elementID[this.domlastActiveTp]);
    },
    actionsByNode(nodo,prefix,clk=null){
        ...
        if(timeDiff < 115){
            ...
            if(clk == null || clk.which == 1){
                ...
                /* Centro el contendor con el div seleccionado */
                var pref = (prefix === 'pri_' ? 'gen_' : prefix );
                topology.centerNodeSection(`#tp_layout_${ pref + topology.domlastActiveTp }`,nodoId);
            }else if(clk.which == 3){
        ...
        // if(prefix==='deta_' || (typeof obj.showDetail == 'undefined' || obj.showDetail==false) ){
            ...
            $(id).draggable({
                ...
                stop: function(e) {
                    ...
                    topology.log_error(['stop',e]);
                    timeFinal = new Date();
                    var timeDiff = timeFinal - timeIni; //en ms
                    if(isNaN(timeDiff)){
                        timeDiff = 0;
                    }
                    if(timeDiff > 115){
                    ...
                    topology.centerNodeSection(`#tp_layout_${ pref + topology.domlastActiveTp }`,id);
                }
            });
        // }
    },
    ...
    minimalParentsNode(statusNde,encodeData,labelNode,idContainerNodes,prefixLayout,child,imgCssNde){
        ...
        var node_ = `<li data-node="${dataPadre}" id="parent_li_${idtpUlNode}" class="li_tp_node">
                        <div style="${colorN}" data-tpid="${this.domlastActiveTp}" data-prefix="${prefixLayout}" data-parentnodetp="#li_" onmousedown="topology.iniPress()" onmouseup="topology.actionsByNode(this,'${prefixLayout}',event)" id="${newPadreId}" class="tp_node not-parents"><span ${imgCssNdePadre} >${deco.title}</span></div>
                        <ul id="${idtpUlNode.replace('pri_','deta_')}" class="tp_graph">
                            ...
                            <li data-node="${encodeData}" id="parent_li_${idContainerNodes}" class="li_tp_node">
                                <div ${statusNde} data-tpid="${this.domlastActiveTp}" data-prefix="${prefixLayout}" data-parentnodetp="#${newPadreId}" onmousedown="topology.iniPress()" onmouseup="topology.actionsByNode(this,'${prefixLayout}',event)" id="li_${idContainerNodes}" class="tp_node"><span ${imgCssNde}>${labelNode}</span></div>
                                ${ul}
                            </li>
